...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            List<T> temp=new ArrayList<T>(copyPath.dataSequence);
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        //Checks that start and end are both vertexes
        if(vertices.get(start)==null)
            throw new NoSuchElementException("No vertex contains start");
        else if(vertices.get(end)==null)
            throw new NoSuchElementException("No vertex contains end");
        PriorityQueue queue=new PriorityQueue();
        Path p=new Path(vertices.get(start));
        queue.add(p); 
        //holds all paths that lead to end vertex
        List<Path> ends=new ArrayList<Path>();
        //holds all visited vertexes
        List<Vertex> visited=new ArrayList<Vertex>();
        //holds the lowest weight path to get to each node
        List<Path> lowest=new ArrayList<Path>();
        while(!queue.isEmpty())
        {
            //Dequeues priority queue
            Path hold=(Path) queue.poll();
            //Checks that hold is not a complete path
            if(hold.end.equals(vertices.get(end)))
            {
                ends.add(hold);
                continue;
            }
            //Adds current vertex to visited
            visited.add(hold.end);
            //Gets size of the edges list for the vertex
            int size=hold.end.edgesLeaving.size(); 
            for (int i=0;i<size;i++){
                boolean contains=false;
                //Checks that the current vertex being checked is not one that has been visited previously
                for(int j=0;j<visited.size();j++)
                    if (hold.end.edgesLeaving.get(i).target.equals(visited.get(j)))
                        contains=true;
                if(contains)
                    continue; 
                else
                {
                    //Creates path that includes vertex stemming from edgesLeaving
                    Path add=new Path(hold,hold.end.edgesLeaving.get(i));                   
                    boolean lowestUsed=false;
                    //Checks if this new path is the new lowest path to current vertex
                    for(int z=0;z<lowest.size();z++){
                        if(lowest.get(z).end.equals(add.end)){
                            lowestUsed=true;
                            //If this path is new lowest, it replace the lowest path for this vertex and adds it to queue
                            if(lowest.get(z).distance>add.distance)
                            {
                                lowest.remove(z);
                                lowest.add(add);
                                queue.add(add);   
                    ...
                    //If the vertex at end of path hasnt been connected to yet, it adds the path to lowest and to priority queue
                    if(!lowestUsed)
                    {
                        lowest.add(add);
                        queue.add(add);
        ...
        //If ends has no elements then no path was found
        if(ends.size()==0)
            throw new NoSuchElementException("There is no path.");
        //sets min to first element of ends
        Path min=ends.get(0);
        //Checks that no other paths in end have a lower value, if the do min is replaced
        for (int i=1;i<ends.size();i++)
            if(ends.get(i).distance<min.distance)
                min=ends.get(i);
        return min;
    }
